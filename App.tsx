...
const App: React.FC = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : '에러 났어. 다시 해보든가.';
      setMessages(prev => prev.map(msg => 
  ...
  return (
    <div className="h-screen bg-amber-50 font-jua text-gray-800 flex flex-col">
      ...
      <footer className="flex-shrink-0 p-4 sm:p-6 lg:p-8 container mx-auto max-w-4xl w-full">
        <PromptInput
          ...
          isLoading={isLoading}
        />
        <p className="text-center text-gray-400 text-xs pt-4">Powered by Google Gemini</p>
      </footer>
    </div>
  );
};
